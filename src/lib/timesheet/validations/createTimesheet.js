...
export const createTimesheetSchema = Joi.object({
  user: objectIdValidator().required().messages({
  ...
  project: objectIdValidator().required().messages({
  ...
  description: Joi.array().items(Joi.string()).default([]).optional(),
  status: Joi.string()
    .valid('pending', 'approved', 'rejected')
    .default('pending')
    .optional(),
  isFinalized: Joi.boolean().default(false).optional(),
